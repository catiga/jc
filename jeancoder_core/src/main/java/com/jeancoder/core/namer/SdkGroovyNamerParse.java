 ...
 *
 */
public class SdkGroovyNamerParse extends NamerParse {
	
	public SdkGroovyNamerParse(){
	...
	public Application parse(NamerApplication namerApplication, String filePath) {
		...
		try {
			...
			if (InstallWay.MEMORY.equals(namerApplication.getInstallWay())) {
				parseMemory(organization, appPath, application, shell);
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 */
	public void forEachDir(String appPath , String organization, Application application, File sourcedir, GroovyShell shell, InstallWay installWay) throws IOException, URISyntaxException {
	 ...
	 * @throws IOException 
	 */
	public void parseMemory(String organization, String appPath, Application application, GroovyShell shell) throws IOException{
	...
	// 因为渲染模板的问题 需要把html前的路径除去
	private ViewResource getViewResource(File entry, String appPath) throws IOException{
        ViewResource viewResource = new ViewResource(FileUtil.getResourcePath(entry.getPath(), appPath), new DynamicResourceType());
